...
export const DoctorProfileView: React.FC<DoctorProfileViewProps> = ({
  ...
  const handleChangePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      alert(t('profile.passwordModal.mismatchAlert'));
      return;
    }
    setPasswordSuccessMsg(t('profile.passwordModal.successMessage'));
    setTimeout(() => {
    ...
    <div className="space-y-6 pb-20 md:pb-10">
          ...
          <div className="text-center sm:text-left space-y-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-blue-50 text-[#4F8EF7] text-xs font-extrabold uppercase border border-blue-100">
                {user.role?.toUpperCase() || t('profile.header.defaultRoleBadge')}
              </span>
              {typeof user.rating === 'number' && (
                <span className="flex items-center gap-1 bg-amber-50 text-amber-700 text-xs font-bold px-2.5 py-0.5 rounded-full border border-amber-200">
                  {user.rating} <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" /> {t('profile.header.reviewsCount', { count: user.reviewCount ?? 0 })}
                </span>
              )}
            </div>
            <p className="text-xs font-semibold text-slate-600">
              {user.specialty || t('profile.header.defaultSpecialty')} • {t('profile.header.licensePrefix', { number: user.licenseNumber || 'MED-DERM-99420' })}
            </p>
            <p className="text-xs text-slate-400">
              {t('profile.header.staffIdPrefix')} <strong className="text-slate-700">{user.staffId || 'REV-STAFF-101'}</strong> {t('profile.header.staffIdSuffix', { branch: selectedBranch.name })}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-2xs space-y-3">
          <h3 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
            ...
            <span>{t('profile.branch.title')}</span>
          </h3>

          <p className="text-xs text-slate-500">
            {t('profile.branch.description')}
          </p>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-2xs space-y-3">
          ...
          >
            ...
            <span>{t('profile.security.changePasswordButton')}</span>
          </button>
        </div>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-2xs space-y-3">
          <h3 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
            ...
            <span>{t('profile.sessions.title')}</span>
          </h3>
      ...
      <div className="pt-4">
        ...
        >
          ...
          <span>{t('profile.logout.signOutButton')}</span>
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 space-y-4 border border-slate-100">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-extrabold text-sm text-slate-900">{t('profile.passwordModal.title')}</h3>
            ...
            {passwordSuccessMsg ? (
              ...
              <form onSubmit={handleChangePasswordSubmit} className="space-y-3 text-xs font-semibold">
                <div>
                  <label className="block text-slate-700 mb-1">{t('profile.passwordModal.currentPasswordLabel')}</label>
                  <input
                    type="password"
                    required
                ...
                <div>
                  <label className="block text-slate-700 mb-1">{t('profile.passwordModal.newPasswordLabel')}</label>
                  <input
                    type="password"
                    required
                    value={newPassword}
                ...
                <div>
                  <label className="block text-slate-700 mb-1">{t('profile.passwordModal.confirmPasswordLabel')}</label>
                  <input
                    type="password"
                    required
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  >
                    {t('profile.passwordModal.updateButton')}
                  </button>
                </div>
              </form>
            )}
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 space-y-4 border border-slate-100 text-center">
            ...
            <h3 className="font-extrabold text-base text-slate-900">{t('profile.logoutModal.title')}</h3>
            <p className="text-xs text-slate-500">
              {t('profile.logoutModal.description')}
            </p>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              >
                {t('profile.logoutModal.stayButton')}
              </button>
              <button
                type="button"
                onClick={onLogout}
              ...
              >
                {t('profile.logoutModal.confirmButton')}
              </button>
            </div>
          </div>
        </div>
